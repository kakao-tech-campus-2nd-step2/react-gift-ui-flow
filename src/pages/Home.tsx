...
const Home: React.FC = () => {
  ...
  const categories = ['생일', '졸업선물', '스몰럭셔리', '명품선물', '결혼/집들이', '따뜻한 선물', '가벼운선물', '팬심저격', '교환권', '건강/비타민', '과일/한우', '출산/키즈'];

  return (
    <div className="home-container">
      <section className="select-section">
        <div className='sub'>
          <img src="https://gift-s.kakaocdn.net/dn/gift/images/m640/bg_profile_default.png" onClick={handleSelectFriend} alt="Select Friend" />
          <h2>선물 받을 친구를 선택해주세요.</h2>
        </div>
      </section>
      <section className="category-section">
        {categories.map(category => (
          <Link to={`/theme/${toUrlFriendly(category)}`} className="category-item" key={category}>
