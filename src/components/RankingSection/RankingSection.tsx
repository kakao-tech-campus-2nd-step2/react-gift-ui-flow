...
const RankingSection = () => {
    ...
    const generateDummyItems = () => {
        const dummyItems = [];
        for (let i = 1; i <= 21; i++) {
            dummyItems.push({ rankingIndex: i, subtitle: "BBQ", title: "BBQ 양념치킨+크림치즈볼+콜라1.25L", amount: 29000, imageSrc: ranking01 });
            ...
            <div className='rankingItemContainer'>
                {visibleItems.map((item) => (
                    <div className='rankingItem'>
                         <RankingGoodsItems rankingIndex={item.rankingIndex} imageSrc={item.imageSrc} subtitle={item.subtitle} title={item.title} amount={item.amount} />
                    </div>
                ))}
            </div>
